  ...
  return (
    <div className="brutal !shadow-[5px_5px_0px_#A78BFA] p-5">
      ...
      <div className="mb-5">
        ...
        <div className="brutal-inset h-2.5 w-full overflow-hidden">
      ...
      {showGraduateButton && (
        <div className="mb-4">
          <button onClick={handleGraduate} disabled={txStatus === "pending"} className="brutal-sm w-full !bg-accent py-3 font-bold !text-background transition-transform hover:-translate-y-0.5 active:translate-y-0.5 disabled:opacity-50">
      ...
      {isUpcoming ? (
        <div className="brutal-inset p-6 text-center">
        ...
        <div className="space-y-4">
          <div className="brutal-inset border-2 border-profit p-4 text-center">
          ...
          <div className="flex gap-3">
            <a href={`https://app.uniswap.org/swap?inputCurrency=ETH&outputCurrency=${tokenAddress}`} target="_blank" rel="noopener noreferrer" className="brutal-sm flex-1 !bg-accent py-3 text-center text-sm font-bold !text-background transition-transform hover:-translate-y-0.5 active:translate-y-0.5">Trade on Uniswap</a>
            {dexPairAddress && <a href={`https://app.uniswap.org/pool/${dexPairAddress}`} target="_blank" rel="noopener noreferrer" className="brutal-sm flex-1 py-3 text-center text-sm font-bold text-muted transition-transform hover:-translate-y-0.5 active:translate-y-0.5">View Pool</a>}
          </div>
        </div>
      ) : !showGraduateButton ? (
        <>
          <div className="brutal-inset mb-4 flex p-1">
            ...
            <button onClick={() => { setMode("sell"); setTxStatus("idle"); setTxHash(null); }} className={`flex-1 rounded-lg py-2 text-sm font-bold transition-colors ${mode === "sell" ? "bg-loss-subtle text-loss" : "text-muted hover:text-foreground"}`}>Sell</button>
          </div>
          ...
          <div className="mb-4">
            ...
            <input type="number" value={amount} onChange={(e) => { setAmount(e.target.value); setTxStatus("idle"); setTxHash(null); }} className="brutal-inset w-full px-4 py-2.5 font-mono text-sm outline-none focus:border-accent" placeholder={mode === "buy" ? "0.1" : "1000"} min={0} step={mode === "buy" ? 0.01 : 1} />
          </div>

          {mode === "buy" && (
            <div className="mb-4 flex gap-2">
              {[0.01, 0.05, 0.1, 0.5].map((v) => (
                <button key={v} onClick={() => setAmount(v.toString())} className="brutal-inset flex-1 py-1.5 text-xs font-bold text-muted transition-colors hover:text-foreground">{v}</button>
              ))}
            </div>
          )}

          {preview && (
            <div className="brutal-inset mb-4 space-y-1.5 p-3 text-xs">
          ...
          {txStatus === "pending" && <div className="brutal-sm mb-4 !border-accent bg-accent-subtle p-3 text-xs font-bold text-accent !shadow-none">Transaction pending… {txHash && `(${txHash.slice(0, 10)}...)`}</div>}
          {txStatus === "success" && <div className="brutal-sm mb-4 !border-profit bg-profit-subtle p-3 text-xs font-bold text-profit !shadow-none">Confirmed. {txHash && <a href={`https://sepolia.etherscan.io/tx/${txHash}`} target="_blank" rel="noopener noreferrer" className="underline">View tx</a>}</div>}
          {txStatus === "error" && <div className="brutal-sm mb-4 !border-loss bg-loss-subtle p-3 text-xs font-bold text-loss !shadow-none">Transaction failed. Try again.</div>}

          <button onClick={handleSubmit} disabled={!amount || Number(amount) <= 0 || txStatus === "pending"} className={`brutal-sm w-full py-3 text-sm font-bold transition-transform hover:-translate-y-0.5 active:translate-y-0.5 disabled:opacity-50 ${mode === "buy" ? "!bg-profit !text-background" : "!bg-loss !text-white"}`}>
