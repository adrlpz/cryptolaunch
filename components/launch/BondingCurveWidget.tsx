    ...
    <div className="rounded-lg border border-edge bg-surface p-5">
      ...
      <div className="mb-5">
        <div className="mb-2 flex items-center justify-between text-xs">
          ...
          <span className={`font-mono ${progress >= 100 ? "text-profit" : "text-accent"}`}>
            {progress.toFixed(1)}%
          </span>
        </div>
        <div className="h-1.5 w-full overflow-hidden rounded-full bg-raised">
          <div
            className={`h-full rounded-full transition-all ${
              progress >= 100 ? "bg-profit" : "bg-accent"
            }`}
      ...
      {showGraduateButton && (
        <div className="mb-4">
          <button
            ...
            className="w-full rounded-lg bg-accent py-3 font-display font-bold text-background transition-opacity hover:opacity-90 disabled:opacity-50"
          >
            {txStatus === "pending" ? "Graduating..." : "Graduate to DEX"}
          </button>
          <p className="mt-2 text-center text-xs text-muted">
            Cap reached — add liquidity to Uniswap and burn LP tokens.
          </p>
        </div>
      )}

      {isUpcoming ? (
        <div className="rounded-lg border border-edge bg-raised p-6 text-center">
        ...
        <div className="space-y-4">
          <div className="rounded-lg border border-profit/30 bg-profit-subtle p-4 text-center">
            <div className="font-display text-lg font-bold text-profit">
              Graduated
            </div>
            <p className="mt-1 text-sm text-muted">
              Trading on Uniswap V2. Liquidity permanently locked.
            </p>
          </div>
          <div className="flex gap-3">
            <a
              ...
              className="flex-1 rounded-lg bg-accent py-3 text-center font-display text-sm font-bold text-background transition-opacity hover:opacity-90"
            >
              Trade on Uniswap
            </a>
            {dexPairAddress && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="flex-1 rounded-lg border border-edge py-3 text-center text-sm text-muted transition-colors hover:text-foreground"
              >
                View Pool
              </a>
            )}
        ...
        <>
          {/* Buy/Sell toggle */}
          <div className="mb-4 flex rounded-lg border border-edge bg-raised p-1">
            <button
          ...
          <div className="mb-4">
            ...
            <input
              ...
              className="w-full rounded-lg border border-edge bg-raised px-4 py-2.5 text-sm font-mono outline-none focus:border-accent"
            ...
            <div className="mb-4 flex gap-2">
              {[0.01, 0.05, 0.1, 0.5].map((v) => (
                <button
                  ...
                  className="flex-1 rounded-md border border-edge bg-raised py-1.5 text-xs text-muted transition-colors hover:text-foreground"
                >
                  {v}
                </button>
              ))}
          ...
          {/* Preview */}
          {preview && (
            <div className="mb-4 space-y-1.5 rounded-lg border border-edge bg-raised p-3 text-xs">
              {[
          ...
          {/* TX status */}
          {txStatus === "pending" && (
            <div className="mb-4 rounded-lg border border-accent/20 bg-accent-subtle p-3 text-xs text-accent">
              Transaction pending… {txHash && `(${txHash.slice(0, 10)}...)`}
            </div>
          )}
          {txStatus === "success" && (
            <div className="mb-4 rounded-lg border border-profit/20 bg-profit-subtle p-3 text-xs text-profit">
              Confirmed.{" "}
              {txHash && (
                <a
          ...
          {txStatus === "error" && (
            <div className="mb-4 rounded-lg border border-loss/20 bg-loss-subtle p-3 text-xs text-loss">
          ...
          <button
            ...
            className={`w-full rounded-lg py-3 font-display text-sm font-bold transition-opacity hover:opacity-90 disabled:opacity-50 ${
              mode === "buy"
