    ...
    <div className="clay p-5">
      ...
      <div className="mb-4 grid grid-cols-2 gap-3">
        <div className="clay-inset p-3">
          <div className="text-xs font-semibold text-muted">Current Price</div>
          <div className="mt-1 font-mono text-lg font-black text-accent">${currentPrice.toFixed(6)}</div>
        </div>
        <div className="clay-inset p-3">
          <div className="text-xs font-semibold text-muted">Total Sold</div>
          <div className="mt-1 font-mono text-lg font-black">{Number(totalSold).toLocaleString()}</div>
        </div>
      </div>

      <div className="mb-5">
        <div className="mb-2 flex items-center justify-between text-xs">
          <span className="font-semibold text-muted">Graduation Progress</span>
          <span className={`font-mono font-bold ${progress >= 100 ? "text-profit" : "text-accent"}`}>{progress.toFixed(1)}%</span>
        </div>
        <div className="clay-inset h-2.5 w-full overflow-hidden">
      ...
      {showGraduateButton && (
        <div className="mb-4">
          <button onClick={handleGraduate} disabled={txStatus === "pending"} className="clay-sm w-full !bg-accent py-3 font-extrabold text-white transition-transform hover:scale-[1.01] active:scale-[0.99] disabled:opacity-50">
            {txStatus === "pending" ? "Graduating..." : "Graduate to DEX"}
          </button>
          <p className="mt-2 text-center text-xs font-semibold text-muted">Cap reached — add liquidity to Uniswap and burn LP tokens.</p>
        </div>
      )}

      {isUpcoming ? (
        <div className="clay-inset p-6 text-center">
          <div className="text-lg font-black text-accent">Trading Not Started</div>
          <p className="mt-2 text-sm font-semibold text-muted">
            Begins <span className="font-mono font-bold text-foreground">{new Date(launchDate!).toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric", hour: "2-digit", minute: "2-digit", timeZoneName: "short" })}</span>
          </p>
        </div>
      ) : isGraduated ? (
        <div className="space-y-4">
          <div className="clay-inset border-2 border-profit/30 p-4 text-center">
            <div className="text-lg font-black text-profit">Graduated</div>
            <p className="mt-1 text-sm font-semibold text-muted">Trading on Uniswap V2. Liquidity permanently locked.</p>
          </div>
          <div className="flex gap-3">
            <a href={`https://app.uniswap.org/swap?inputCurrency=ETH&outputCurrency=${tokenAddress}`} target="_blank" rel="noopener noreferrer" className="clay-sm flex-1 !bg-accent py-3 text-center text-sm font-extrabold text-white transition-transform hover:scale-[1.01] active:scale-[0.99]">Trade on Uniswap</a>
            {dexPairAddress && <a href={`https://app.uniswap.org/pool/${dexPairAddress}`} target="_blank" rel="noopener noreferrer" className="clay-sm flex-1 py-3 text-center text-sm font-bold text-muted transition-colors hover:text-foreground">View Pool</a>}
          </div>
        </div>
      ) : !showGraduateButton ? (
        <>
          <div className="clay-inset mb-4 flex p-1">
            <button onClick={() => { setMode("buy"); setTxStatus("idle"); setTxHash(null); }} className={`flex-1 rounded-2xl py-2 text-sm font-bold transition-all ${mode === "buy" ? "!bg-profit-subtle text-profit shadow-sm" : "text-muted hover:text-foreground"}`}>Buy</button>
            <button onClick={() => { setMode("sell"); setTxStatus("idle"); setTxHash(null); }} className={`flex-1 rounded-2xl py-2 text-sm font-bold transition-all ${mode === "sell" ? "!bg-loss-subtle text-loss shadow-sm" : "text-muted hover:text-foreground"}`}>Sell</button>
          </div>
          ...
          <div className="mb-4">
            ...
            <input type="number" value={amount} onChange={(e) => { setAmount(e.target.value); setTxStatus("idle"); setTxHash(null); }} className="clay-inset w-full px-4 py-2.5 font-mono text-sm font-bold outline-none focus:ring-2 focus:ring-accent/30" placeholder={mode === "buy" ? "0.1" : "1000"} min={0} step={mode === "buy" ? 0.01 : 1} />
          </div>

          {mode === "buy" && (
            <div className="mb-4 flex gap-2">
              {[0.01, 0.05, 0.1, 0.5].map((v) => (
                <button key={v} onClick={() => setAmount(v.toString())} className="clay-inset flex-1 py-1.5 text-xs font-bold text-muted transition-colors hover:text-foreground">{v}</button>
              ))}
            </div>
          )}
          ...
          {preview && (
            <div className="clay-inset mb-4 space-y-1.5 p-3 text-xs">
              {[
                ["You pay", preview.input, "font-semibold"],
                ["You receive", preview.output, "font-bold text-profit"],
                ["Fee (1%)", preview.fee, "font-bold text-loss"],
                ["Price Impact", preview.priceImpact, "font-semibold"],
              ].map(([label, value, color]) => (
                <div key={label} className="flex justify-between"><span className="font-semibold text-muted">{label}</span><span className={color}>{value}</span></div>
              ))}
            </div>
          )}

          {txStatus === "pending" && <div className="clay-sm mb-4 !bg-accent-subtle p-3 text-xs font-bold text-accent">Transaction pending… {txHash && `(${txHash.slice(0, 10)}...)`}</div>}
          {txStatus === "success" && <div className="clay-sm mb-4 !bg-profit-subtle p-3 text-xs font-bold text-profit">Confirmed. {txHash && <a href={`https://sepolia.etherscan.io/tx/${txHash}`} target="_blank" rel="noopener noreferrer" className="underline">View tx</a>}</div>}
          {txStatus === "error" && <div className="clay-sm mb-4 !bg-loss-subtle p-3 text-xs font-bold text-loss">Transaction failed. Try again.</div>}

          <button onClick={handleSubmit} disabled={!amount || Number(amount) <= 0 || txStatus === "pending"} className={`clay-sm w-full py-3 text-sm font-extrabold transition-transform hover:scale-[1.01] active:scale-[0.99] disabled:opacity-50 ${mode === "buy" ? "!bg-profit text-white" : "!bg-loss text-white"}`}>
