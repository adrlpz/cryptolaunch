...
export default function MarginCalculator({ tokenName, tokenSymbol, tokenPrice, maxLeverage = 50, onOpenPosition }: MarginCalculatorProps) {
  ...
  return (
    <div className="clay p-5">
      ...
      <div className="mb-4">
        <label className={fieldLabel}>Collateral (USDT)</label>
        <input type="number" value={modal} onChange={(e) => setModal(Number(e.target.value) || 0)} min={1} className="clay-inset w-full px-4 py-2.5 font-mono text-sm outline-none focus:ring-2 focus:ring-accent/40" placeholder="100" />
      </div>

      <div className="mb-5">
        <label className={fieldLabel}>Leverage ({MARGIN_FEE_PERCENT}% fee on debt)</label>
        <div className="flex gap-1.5">
          {availableLevels.map((level) => (
            <button key={level} onClick={() => setLeverage(level)} className={`flex-1 rounded-xl py-2 text-xs font-bold transition-colors ${
              leverage === level ? "bg-accent text-background" : "clay-inset text-muted hover:text-foreground"
            }`}>{level}%</button>
          ))}
        </div>
      </div>
      ...
      <div className="clay-inset mb-5 p-4">
        ...
        <div className="mt-2 pt-2">
          {[`${tokenSymbol} amount`, calc.coinsPurchased.toFixed(2)].length > 0 && (
            <>
              <div className="flex items-center justify-between py-1.5 text-xs"><span className="text-muted">{tokenSymbol} amount</span><span className="font-mono">{calc.coinsPurchased.toFixed(2)}</span></div>
              <div className="flex items-center justify-between py-1.5 text-xs"><span className="text-muted">Entry price</span><span className="font-mono">${tokenPrice.toFixed(6)}</span></div>
            </>
          )}
        </div>
      </div>

      <div className="clay-inset mb-5 border border-loss/20 p-4">
        ...
        {[
          ...
          <div key={label} className="flex items-center justify-between py-1 text-xs"><span className="text-muted">{label}</span><span className="font-mono text-loss">{value}</span></div>
        ))}
      </div>
      ...
      <button onClick={() => onOpenPosition?.(modal, leverage)} className="clay-sm w-full bg-accent py-3 text-sm font-bold text-background transition-opacity hover:opacity-90">
