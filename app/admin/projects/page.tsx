    ...
    <div className="mx-auto max-w-7xl px-4 py-10">
      ...
      {loading ? (
        <div className="space-y-2">
          {[1, 2, 3].map((i) => <div key={i} className="clay h-16 animate-pulse" />)}
        </div>
      ) : (
                ...
                {projects.map((p) => (
                  <tr key={p.id} className="border-b border-edge/30">
                    <td className="px-4 py-3">
                      <Link href={`/projects/${p.id}`} className="font-bold hover:text-accent">
                        {p.tokenName} <span className="text-muted">${p.tokenSymbol}</span>
                      </Link>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="clay-inset h-1.5 w-12 overflow-hidden">
                          <div className="h-full rounded-full bg-accent" style={{ width: `${p.soldPercent}%` }} />
                        </div>
                        <span className="text-xs text-muted">{p.soldPercent.toFixed(0)}%</span>
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span className={`rounded-xl px-2 py-0.5 text-xs font-bold ${
                        ...
                        p.lpStatus === "bonding" ? "bg-accent-subtle text-accent" : "bg-raised text-muted"
                      }`}>{p.lpStatus}</span>
                    </td>
                    <td className="px-4 py-3">
                      <select value={p.status} onChange={(e) => handleStatusChange(p.id, e.target.value)}
                        className="clay-inset rounded-lg px-2 py-1 text-xs outline-none">
                    ...
                    <td className="px-4 py-3">
                      <Link href={`/projects/${p.id}`} className="text-xs text-accent hover:underline">View</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
