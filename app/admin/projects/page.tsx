    ...
    <div className="mx-auto max-w-7xl px-4 py-10">
      <div className="mb-8">
        ...
        <div className="flex items-end justify-between">
          ...
          <span className="text-xs font-semibold text-muted">{projects.length} total</span>
        </div>
      </div>

      {loading ? (
        <div className="space-y-2">{[1, 2, 3].map((i) => <div key={i} className="clay h-16 animate-pulse" />)}</div>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-edge">
                  {["Token", "Chain", "Price", "Sold", "Positions", "LP", "Status", ""].map((h) => (
                ...
                {projects.map((p) => (
                  <tr key={p.id} className="border-b border-edge/30">
                    <td className="px-4 py-3">
                      <Link href={`/projects/${p.id}`} className="font-extrabold hover:text-accent">{p.tokenName} <span className="font-semibold text-muted">${p.tokenSymbol}</span></Link>
                    </td>
                    <td className="px-4 py-3 text-xs font-semibold text-muted">{p.chain.toUpperCase()}</td>
                    <td className="px-4 py-3 font-mono text-xs font-bold">${Number(p.tokenPrice).toFixed(6)}</td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <div className="clay-inset h-2 w-12 overflow-hidden">
                          <div className="h-full rounded-full bg-accent" style={{ width: `${p.soldPercent}%` }} />
                        </div>
                        <span className="text-xs font-semibold text-muted">{p.soldPercent.toFixed(0)}%</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 font-bold">{p.positionCount}</td>
                    <td className="px-4 py-3">
                      <span className={`rounded-2xl px-2 py-0.5 text-xs font-bold ${
                        p.lpStatus === "graduated" ? "bg-profit-subtle text-profit" :
                        p.lpStatus === "bonding" ? "bg-accent-subtle text-accent" : "clay-inset text-muted"
                      }`}>{p.lpStatus}</span>
                    </td>
                    <td className="px-4 py-3">
                      <select value={p.status} onChange={(e) => handleStatusChange(p.id, e.target.value)} className="clay-inset rounded-xl px-2 py-1 text-xs font-semibold outline-none">
