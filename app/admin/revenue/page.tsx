...
export default function AdminRevenuePage() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10">
      ...
      {loading ? (
        <div className="animate-pulse space-y-4"><div className="glass h-28" /><div className="glass h-56" /></div>
      ) : data ? (
        <>
          <div className="glass-glow mb-6 p-6 text-center">
          ...
          <div className="mb-6 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {[
              { label: "Margin Fee", value: fmt(data.revenue.marginFee.total), pct: data.breakdown.marginPercent },
              { label: "Platform Fee", value: fmt(data.revenue.platformFee), pct: data.breakdown.platformPercent },
              { label: "Trading Fee", value: fmt(data.revenue.tradingFee), pct: data.breakdown.tradingPercent },
              { label: "Deploy Fee", value: fmt(data.revenue.deployFee), pct: data.breakdown.deployPercent },
            ].map((card) => (
          ...
          <div className="glass mb-6 p-5">
            <h2 className="mb-4 text-xs uppercase tracking-wider text-muted">Distribution</h2>
            <div className="space-y-3">
              {[{ label: "Margin Fee", pct: data.breakdown.marginPercent }, { label: "Platform Fee", pct: data.breakdown.platformPercent }, { label: "Trading Fee", pct: data.breakdown.tradingPercent }, { label: "Deploy Fee", pct: data.breakdown.deployPercent }].map((item) => (
                <div key={item.label}>
                  ...
                  <div className="h-1.5 w-full overflow-hidden rounded-full bg-edge"><div className="h-full rounded-full bg-accent" style={{ width: `${Math.min(100, item.pct)}%` }} /></div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="glass-glow p-5">
            <h2 className="mb-3 text-xs uppercase tracking-wider text-muted">Insurance Fund</h2>
        ...
        <div className="glass p-10 text-center"><p className="text-sm text-muted">Failed to load revenue data.</p></div>
      )}
    </div>
  );
}
